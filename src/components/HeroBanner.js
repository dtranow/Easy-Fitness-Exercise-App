...
const HeroBanner = ({ toggleChat }) => {
  return (
    <Box position='relative' p='18px' sx={{mt: { lg: '100px', xs: '44px'}, ml: {sm: '50px'}}}>
      <Typography className='hero-banner-title' fontWeight='800' color='red' fontSize='32px' mb={3} >
        Learn to Exercise
      </Typography>
      <Typography fontWeight='800' fontSize='32px' mb={4}>
        Improve your health <br /> and well-being with <br /> proper instructions
      </Typography>
      <Button href='#exercises' variant='contained' color='error' sx={{ width: '220px', height: '45px', fontSize:'16px', '&:hover': 
        { backgroundColor: 'darkred' }}}>Explore Exercises</Button>
      <Typography fontWeight='800' fontSize='32px' mb={4} mt={4}>
        Ask exercise questions<br /> to our AI chatbot
      </Typography>
      <Button onClick={toggleChat} variant='contained' color='error' sx={{ width: '220px', height: '45px', fontSize:'16px', '&:hover':
        { backgroundColor: 'darkred' }}}>Ask Questions</Button>
